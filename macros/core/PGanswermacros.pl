...
sub function_invalid_params {
	...
	my $error_response = sub {
		my $PGanswerMessage =
			"Tell your professor that there is an error with the parameters " . "to the function answer evaluator";
...
sub compare_numbers {
	...
	$rh_ans->{score} = 1 if (($is_a_number)
		and (abs($inVal - $rh_ans->{correct_ans}) <= $permitted_error));
	if (not $is_a_number) {
...
sub std_num_array_filter {
	my $rh_ans  = shift;
	my %options = @_;
	set_default_options(\%options, '_filter_name' => 'std_num_array_filter',);
...
sub function_from_string2 {
	...
	my ($function_sub, $PG_eval_errors, $PG_full_errors) = PG_answer_eval(
		q!
	    sub {
	    	my @VARS = @_;
	    	my $input_str = '';
...
sub is_zero_array {
	...
	if (not is_a_number($max)) {
		...
		$rh_ans->{ $options{stdout} } =
			($max < $options{tolerance}) ? 1 : 0;    # set 'score' to 1 if the array is close to 0;
	}
	$rh_ans;
}
...
sub calculate_difference_vector {
	...
	while (@evaluation_points) {
		...
		$errors .= " $err1 " if defined($err1);
		$errors .= " Error detected evaluating student input at (" . join(' , ', @vars) . " ) "
			if defined($options{debug})
			and $options{debug} == 1
			and defined($err1);
		($correctVal, $err2) = &{$rf_correct_fun}(@inputs);
		$errors .= " There is an error in WeBWorK's answer to this problem, please alert your instructor.<br> $err2 "
			if defined($err2);
		$errors .= " Error detected evaluating correct adapted answer  at (" . join(' , ', @inputs) . " ) "
			if defined($options{debug})
			and $options{debug} = 1
			and defined($err2);
		($instructorVal, $err3) = &$rf_correct_fun(@vars, @zero_params);
		$errors .= " There is an error in WeBWorK's answer to this problem, please alert your instructor.<br> $err3 "
			if defined($err3);
		$errors .= " Error detected evaluating instructor answer  at (" . join(' , ', @vars, @zero_params) . " ) "
			if defined($options{debug})
			and $options{debug} = 1
			and defined($err3);
		...
		unless (defined($err1) or defined($err2) or defined($err3)) {
			...
			if ($options{tolType} eq 'relative') {    #relative tolerance
				...
				$diff = ($inVal - ($correctVal - $instructorVal)) / $instructorVal - 1
					if abs($instructorVal) > $options{zeroLevel};
...
sub is_an_arithmetic_expression {
	...
	if ($process_ans_hash) {
		if ($is_an_arithmetic_expression == 1) {
			...
			$rh_ans->{student_ans} =
				"Not an arithmetic expression: You must enter an arithmetic expression, e.g. -6 or (2.3*4+5/3)^2";
...
sub check_syntax {
	...
	if (ref($ret)) {                                ## parsed successfully
		...
		$rh_ans->{ $options{stdout} } = 'syntax error:' . $parser->{htmlerror},
			...
			$rh_ans->throw_error('SYNTAX',
				'syntax error in answer:' . $parser->{htmlerror} . "$BR" . $parser->{error_msg});
...
# last: 'STRING' post_filter will clear the error (avoiding pink screen.)
	...
	$rh_ans->{'preview_text_string'} = $ans, $rh_ans->{'preview_latex_string'} = $ans,
...
sub check_units {
	...
	unless (defined($num_answer) && $units) {
		...
		if ($ans =~ /\S/) {    # the answer is not blank
			...
			$rh_ans->throw_error('UNITS',
				"The answer \"$ans\" could not be interpreted "
					. "as a number or an arithmetic expression followed by a unit specification. "
					. "Your answer must contain units.");
		}
		return $rh_ans;
	...
	if ($units_match) {
		...
		$rh_ans->{'tolerance'} = $rh_ans->{'tolerance'} * $correct_units{'factor'} / $units{'factor'}
			if $rh_ans->{'tolType'} eq 'absolute';    # the tolerance is in the units specified by the instructor.
