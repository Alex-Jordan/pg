...
sub urand {    # generate normally dist. random numbers
	...
	while ($N > 0) {
		...
		my @r      = (
			significant_decimals($mean + $sd * $radius * CORE::sin($angle), $digits),
...
sub exprand {    # generate exponentially dist. numbers  Exp(x,lambda)
	...
	while ($N > 0) {
		...
		$N -= 1;
		push(@numbers,
			significant_decimals(-log($main::PG_random_generator->random(0.0, 1.0, 0.0)) / $lambda, $digits));
	}
...
#
	...
	if ($args{'variance'} eq "separate") {
		...
		$t =
			($sum_x / $nx - $sum_y / $ny) /
			sqrt(($sum_squares_x - $sum_x * $sum_x / $nx) / ($nx * ($nx - 1.0)) +
				($sum_squares_y - $sum_y * $sum_y / $ny) / ($ny * ($ny - 1.0)));
	} else {
		# Use the pooled variance formula to calculate the t statistic
		$t =
			($sum_x / $nx - $sum_y / $ny) /
			sqrt(($sum_squares_x - $sum_x * $sum_x / $nx + $sum_squares_y - $sum_y * $sum_y / $ny) /
				($nx + $ny - 2.0) *
				(1.0 / $nx + 1.0 / $ny));
	}
