...
sub Break {
	...
	if ($self->{ignoreNL}) {
		...
		if   ($top->{breakInside}) { $top->pushText($token) }
...
sub Brace {
	...
	my $token = shift;
	my $top   = $self->{block}->topItem;
	if ($top->{options}) { $self->Begin($token, ' {') }
	else                 { $self->Text($token) }
}
...
sub Star {
	...
	if   ($self->{block}{parseAll}) { $self->Emphasis($token) }
	else                            { $self->Text($token) }
}
...
sub Bullet {
	...
	$bullet = { '*' => 'disc', '+' => 'square', 'o' => 'circle', '-' => 'bullet' }->{ substr($token, 0, 1) }
		if $bullet eq 'bullet';
	my $block = $self->{block};
...
sub combineTopItems {
	...
	my $top  = $self->topItem($i);
	my $prev = $self->topItem($i - 1);
	my $par;
	for (my $j = $i - 1; $prev->{type} eq 'par' && $top->{combine}{par}; $j--) {
		$par  = $prev;
		$prev = $self->topItem($j);
	}
	my $id = $top->{combine}{ $prev->{type} };
	my $value;
	my $inside = 0;
	if ($id) {
		if (ref($id) eq 'HASH') { ($id, $value) = %$id; $inside = 1 }
		else                    { $value = $prev->{$id} }
		...
		if (
			...
			|| (
				...
				&& $top->{bullet} eq 'roman'
				&& $prev->{type} eq 'list'
				&& $prev->{bullet} eq 'alpha'
				&& (($topList eq 'i.' && $prevList eq 'h.')
					|| ($topList eq 'v.' && $prevList eq 'u.')
					|| ($topList eq 'x.' && $prevList eq 'w.')
					|| ($topList eq 'l.' && $prevList eq 'k.'))
			)
			|| (
				$top->{type} eq 'list'
				&& $top->{bullet} eq 'Roman'
...
sub Code {
	...
	my $code =
		...
		. join("<\/cline>\n<cline>", split(/\n/, $self->string($item)))
		. "<\/cline>\n<\/pre>\n"
		: $self->nl . "<pre>" . $self->string($item) . "<\/pre>\n";
...
sub Format {
	...
	main::WARN_MESSAGE("==================", "Errors parsing PGML:", @warnings, "==================")
		if scalar(@warnings);
	return $format;
}
