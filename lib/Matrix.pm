...
sub _stringify {
	...
	for ($i = 0; $i < $rows; $i++) {
		$s .= "[ ";
		for ($j = 0; $j < $cols; $j++) {    #warn " i $i j $j ",$object->rh_options;
			my $format =
				(defined($object->rh_options->{display_format}))
				? $object->rh_options->{display_format}
				: $Matrix::DEFAULT_FORMAT;
			$s .=
				(ref($object->[0][$i][$j]) =~ /Complex/)
				? " " . $object->[0][$i][$j]->stringify_cartesian . " "
				:    #FIXME
...
=head2 new_row_matrix [DEPRECATED]
...
sub proj_coeff {
	...
	warn
		"A unique adapted answer could not be determined.  Possibly the parameters have coefficient zero.<br>  dim = $dim base_matrix is $base_matrix\n"
		if $dim;    # only print if the dim is not zero.
	$x_vector;
}

=head2 new_column_matrix
...
=head1 Overrides of MatrixReal which allow use of complex entries
